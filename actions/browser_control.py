...
class _BrowserSession:
    ...
    async def _launch(self):
        ...
        if engine_name == "firefox":
            profile = _firefox_profile_dir() or str(
                Path.home() / ".dodol_profiles" / "firefox"
            )
            kwargs: dict = {
                "headless":    False,
                "slow_mo":     0,
            ...
            try:
                ...
                dodol_profile = str(Path.home() / ".dodol_profiles" / "firefox_dodol")
        ...
        if engine_name == "webkit":
            safari_profile = str(Path.home() / ".dodol_profiles" / "safari")
        ...
        # automation). Fall back to a persistent DODOL automation profile —
    ...
    async def screenshot(self, path: str = None) -> str:
        page = await self._get_page()
        try:
            save_path = path or str(Path.home() / "Desktop" / "dodol_screenshot.png")
