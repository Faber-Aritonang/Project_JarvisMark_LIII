"""
Drop-in DODOL plugin template.

Copy this file, rename it (no leading underscore), fill in PLUGIN and run().
No other file needs to change — DODOL discovers this automatically at startup.
"""
...
def run(parameters: dict, player=None, session_memory=None) -> str:
    ...
    player: the DodolUI instance — use player.write_log(f"DODOL: ...") to log,
    ...
    if player:
        try:
            player.write_log(f"DODOL: {result_text}")
        except Exception:
            pass
    return result_text
