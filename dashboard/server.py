"""
dashboard/server.py — Dodol Local HTTP Dashboard
...
def _make_uploads_dir() -> Path:
    ...
    for candidate in [
        Path.home() / "Downloads" / "Dodol Uploads",
        Path.home() / "Documents" / "Dodol Uploads",
        BASE_DIR / "uploads",
    ]:
        try:
...
# ── AES-256-CBC ───────────────────────────────────────────────────────────────
_AES_SALT = b'DODOL-DASHBOARD-v1'
...
def _ensure_network_access(port: int) -> None:
    ...
    if sys.platform == "win32":
        ...
        bat_body = "\r\n".join(bat_lines) + "\r\n"
        fd, bat_path = tempfile.mkstemp(suffix=".bat", prefix="dodol_fw_")
        try:
...
class DashboardServer:
    ...
    def _ssl_enabled() -> bool:
        ...
        return (certs / "dodol.key").exists() and (certs / "dodol.crt").exists()
...
<p>Press <strong style="color:#dde3ed">Remote Control</strong> in Dodol to get a new QR code.</p>
</div></body></html>""")
...
<p>Connecting to Dodol…</p>
    ...
    async def _serve_alias(self) -> None:
        ...
        ssl_key  = BASE_DIR / "config" / "certs" / "dodol.key"
        ssl_cert = BASE_DIR / "config" / "certs" / "dodol.crt"
